...
class Spectrum:
    def __init__(self, filename, scan, index, peaks, mz, charge, ms_level, collision_energy=0.0, fragmentation_method="NO_FRAG", precursor_intensity=0.0):
        self.filename = filename
        self.scan = scan
        self.peaks = peaks
        self.mz = mz
...
class PeptideLibrarySpectrum(Spectrum):
    ...
    @staticmethod
    def get_tsv_header():
        return "mgf_filename\toriginalfilename\tspectrumindex\tspectrumscan\tcharge\tmz\tpeptide\tprotein\tcollision_energy\tannotated_peaks\texplained_intensity\tsignal_peaks\tnumber_of_peaks_within_1_percent_of_max\tnumber_of_peaks_within_5_percent_of_max\tpeaks\tannotated_ions\tscore"

    def get_tsv_line(self, output_mgf_filename=""):
        return "%s\t%s\t%d\t%d\t%d\t%f\t%s\t%s\t%f\t%d\t%f\t%d\t%d\t%d\t%d\t%d\t%f" % (output_mgf_filename, self.filename, self.index, self.scan, self.charge, self.mz, self.peptide, self.protein, self.collision_energy, self.annotated_peaks, self.explained_intensity, self.signal_peaks, self.number_of_peaks_within_1_percent_of_max, self.number_of_peaks_within_5_percent_of_max, len(self.peaks), self.annotated_ions, self.score)
...
def calculated_number_unique_ions_annotated_in_signal(peaks, max_charge, peptide, tolerance, SNR=3.0):
