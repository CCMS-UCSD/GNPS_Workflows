...
def main():
    ...
    for dataset in all_matches:
        ...
        for match in all_matches[dataset]["matches"]:
            ...
            output_map['specs_scan'].append(match["queryscan"])
            ...
            output_map['score'].append(match["cosine"])

            #List the library identifications
            if str(match["queryscan"]) in scan_to_library_map:
                output_map['Compound_Name'].append(scan_to_library_map[str(match["queryscan"])]["Compound_Name"])
                output_map['SpectrumID'].append(scan_to_library_map[str(match["queryscan"])]["SpectrumID"])
            else:
